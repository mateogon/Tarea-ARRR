...
def load_measurement_csv(file_name="measurement_results.csv"):
...
def generate_visualizations(df, summary):
    # Gráfico: IPs únicas detectadas por protocolo y destino
    ...
    plt.title("IPs únicas detectadas por protocolo y destino")
    plt.xlabel("Destino")
    plt.ylabel("IPs únicas")
    plt.legend()
    plt.xticks(rotation=45)
    plt.tight_layout()
    plt.savefig("ips_unique_by_protocol.png")
    plt.close()

    # Gráfico: Total de paquetes capturados por protocolo
    ...
    plt.title("Total de paquetes capturados por protocolo")
    plt.xlabel("Protocolo")
    plt.ylabel("Total de paquetes")
    plt.tight_layout()
    plt.savefig("total_packets_by_protocol.png")
    plt.close()
...
def generate_report(df, summary):
    ...
    with open("measurement_report.md", "w") as report_file:
        report_file.write(report)
    print("Reporte generado como 'measurement_report.md'.")

# Main workflow
def main():
    # Cargar resultados
    df = load_measurement_csv()
    ...
    print("Consolidación completada. Visualizaciones y reporte generados.")
